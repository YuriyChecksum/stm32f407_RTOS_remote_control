...
#define hi2c_ath25 &hi2c2
//I2C_HandleTypeDef* hi2c_ath25 = &hi2c2;

uint8_t data[7] = { 0 };

// стартовая последовательность для запуска преобразования
const uint8_t startConversionBytes[3] = { 0xAC, 0x33, 0x00};

// Посылаем start conversion Command {0xAC, 0x33, 0x00} и ждём 80ms
void ATH25_start_conversion() {
	HAL_I2C_Master_Transmit(hi2c_ath25, I2C_ADDR_ATH25 << 1, (uint8_t*)startConversionBytes, 3, 100); // start conversion
...
void ATH25_softReset() {
	...
	HAL_I2C_Master_Transmit(hi2c_ath25, I2C_ADDR_ATH25 << 1, &value, 1, 100);
}
...
void ATH25_init() {
	...
	//Посылаем Initialization Command 0x71
	uint8_t val = 0;
	HAL_StatusTypeDef st = HAL_I2C_Mem_Read(hi2c_ath25, I2C_ADDR_ATH25 << 1, 0x71,
...
HAL_StatusTypeDef ATH25_Read_Data(float* temperature, float* humidity) {
	...
	do {
		...
		HAL_Delay(10);
		HAL_StatusTypeDef st = HAL_I2C_Master_Receive(hi2c_ath25, I2C_ADDR_ATH25 << 1, data, 7, 100);
		if (st != HAL_OK) {
	...
	*temperature = temp_raw / 5242.88f - 50;
	return HAL_OK;
}
